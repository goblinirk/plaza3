...
class UserGroups extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('label, permissions, create_date, status', 'required'),
			array('permissions, status', 'numerical', 'integerOnly'=>true),
			array('label', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, label, permissions, create_date, status', 'safe', 'on'=>'search'),
		);
	}
