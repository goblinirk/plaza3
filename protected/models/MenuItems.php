...
class MenuItems extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('owner_id, label, module, action, params, status', 'required'),
			array('owner_id, status', 'numerical', 'integerOnly'=>true),
			array('label, module, action, params', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, owner_id, label, module, action, params, status', 'safe', 'on'=>'search'),
		);
	}
